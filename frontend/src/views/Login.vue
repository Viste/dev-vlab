...
async function loginVK() {
  try {
    ...
    error.value = 'VK auth error: ' + (e.response?.data?.error || e.message)
  }
}
...
<template>
  <div class="max-w-md mx-auto px-4 py-16">
    <div class="bento-card p-8">
      ...
      <div class="space-y-3">
        <button @click.stop="loginVK" type="button"
          class="relative z-10 w-full flex items-center justify-center gap-3 bg-blue-600/90 hover:bg-blue-600 text-white font-medium py-3 px-4 rounded-xl transition text-sm cursor-pointer">
        ...
        <div id="telegram-login" class="flex justify-center relative z-0"></div>
      </div>
    </div>
  </div>
</template>
